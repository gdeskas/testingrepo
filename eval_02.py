...
# DBTITLE 1,Prompt-to-field mapping (which fields belong to which prompt)
PROMPT_FIELDS = {
    "categorise_doc": [
...
for pname in PROMPTS_TO_EVALUATE:
    fields = PROMPT_FIELDS.get(pname, [])
    print(f"  {pname:30s} → {len(fields)} fields")
    if not fields:
        print(f"    WARNING: No field mapping defined — will use all non-null val_ columns")
...
for prompt_name in PROMPTS_TO_EVALUATE:
    ...
    # Only use val_ columns that belong to this prompt
    allowed_fields = PROMPT_FIELDS.get(prompt_name, [])
    if allowed_fields:
        allowed_val_cols = [f"val_{f}" for f in allowed_fields]
        prompt_val_cols = [c for c in val_columns
                           if c in allowed_val_cols
                           and prompt_runs[c].notna().any()
                           and not (prompt_runs[c] == "None").all()]
    else:
        # Fallback: use all non-null val_ columns (no mapping defined)
        prompt_val_cols = [c for c in val_columns
...
# DBTITLE 1,Batched judge prompt and function
...
def judge_document_batch(fields_with_values: list, document_text: str) -> list:
    ...
    Args:
        fields_with_values: list of dicts with {"field": str, "value": str}
        document_text: the source document text

    Returns:
        list of dicts with {"field", "score", "reasoning"} — one per input field
    """
    try:
        ...
        # Response format: array of {field, score, reasoning}
        resp_fmt = json.dumps({
            "type": "json_schema",
            "json_schema": {
                "name": "judge_batch_response", "strict": True,
                "schema": {
                    "type": "object",
                    "properties": {
                        "results": {
        ...
        # Build lookup by field name for robust matching
        results_by_field = {}
        for r in results_list:
            fname = r.get("field", "")
            results_by_field[fname] = {
                "score": max(0.0, min(1.0, float(r.get("score", 0.0)))),
                "reasoning": str(r.get("reasoning", "")),
            }

        # Match back to input fields (handle case where judge returns different order)
        output = []
        for f in fields_with_values:
            match = results_by_field.get(f["field"], None)
            if match:
                output.append({"field": f["field"], **match})
            else:
                output.append({"field": f["field"], "score": 0.0,
                               "reasoning": "Field not returned by judge"})

        return output

    except Exception as e:
        print(f"    JUDGE BATCH ERROR: {e}")
        return [{"field": f["field"], "score": 0.0, "reasoning": f"Judge failed: {e}"}
                for f in fields_with_values]


print(f"judge_document_batch() defined — using {JUDGE_MODEL}")
print("One LLM call per document (all fields judged together)")
...
# MAGIC
# MAGIC One 120b call per document — all fields for that document judged in a single batch.

# COMMAND ----------

# DBTITLE 1,Main judge loop (batched — one call per document)
...
total_docs = judgeable["document_id"].nunique()

print(f"Eval run: {eval_run_id}")
print(f"Judge model: {JUDGE_MODEL}")
print(f"Total documents to judge: {total_docs}")
print(f"Total field-values: {len(judgeable)}")
print(f"Estimated judge calls: ~{total_docs} (one per document)")
print("=" * 70)
...
for prompt_name in PROMPTS_TO_EVALUATE:
    ...
    print(f"\n{'='*70}")
    print(f"[{prompt_name}] {len(doc_ids)} docs, {len(prompt_modal)} fields — ~{len(doc_ids)} judge calls")
    print(f"{'='*70}")
    ...
    for doc_idx, doc_id in enumerate(doc_ids):
        ...
        # Build batch of all fields for this document
        fields_batch = []
        field_meta = {}  # store consistency info per field for later
        for _, row in doc_fields.iterrows():
            fields_batch.append({"field": row["field"], "value": row["modal_value"]})
            field_meta[row["field"]] = {
                "file_name": row.get("file_name", ""),
                "consistency_rate": row["consistency_rate"],
                "num_runs": row["num_runs"],
                "modal_value": row["modal_value"],
            }

        # ONE judge call for all fields in this document
        batch_results = judge_document_batch(fields_batch, doc_text)
        judge_calls += 1
        ...
        for result in batch_results:
            ...
            all_scores.append({
                ...
                "field": field,
                "modal_value": str(meta.get("modal_value", "")),
                "score": result["score"],
...
print(f"\nTotal: {len(all_scores)} field scores from {judge_calls} judge calls ({total_elapsed:.0f}s)")
