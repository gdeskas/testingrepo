...
def snapshot_silver_tables(case_folder: str, batch_id: str, proposal_job_run_id: str):
    """For each production table, append ONLY this proposal's rows
    (filtered by JOB_RUN_ID) to the corresponding eval_snap_* table with
    traceability columns. Filtering is essential because the production
    Silver tables are append-only and contain historical data from prior
    runs — without the filter we'd snapshot thousands of unrelated rows."""
    for prod_table, snap_table in SNAPSHOT_TABLES.items():
        ...
        try:
            ...
            # Filter to only this proposal's rows
            if "JOB_RUN_ID" in df.columns:
                df = df.filter(F.col("JOB_RUN_ID") == proposal_job_run_id)
            elif "job_run_id" in df.columns:
                df = df.filter(F.col("job_run_id") == proposal_job_run_id)
            else:
                print(f"    {prod_table:<22s}  WARN: no JOB_RUN_ID column — snapshotting full table")
            row_count = df.count()
            if row_count == 0:
            ...
            df_snap = (
                df.withColumn("_source_folder",   F.lit(case_folder))
                  ...
                  .withColumn("_proposal_job_id", F.lit(proposal_job_run_id))
...
for idx, volume_root in enumerate(VOLUME_ROOTS, 1):
    ...
    # Per-proposal job_run_id — this is the key to the snapshot filter.
    # Each proposal's pipeline run gets its own JOB_RUN_ID, so when we
    # snapshot the production tables we can filter to only this proposal's
    # rows (the production tables are append-only and contain history).
    proposal_job_run_id = f"{job_run_id}_p{idx:02d}_{datetime.now():%H%M%S}"

    # Override the per-batch job_run_id with the per-proposal one when
    # calling the pipeline notebooks — they will tag every row with this.
    proposal_pipeline_args = {**pipeline_args, "job_run_id": proposal_job_run_id}
    ...
    try:
        binary_df = (
            spark.read.format("binaryFile")
                 ...
                 .withColumn("job_run_id",       F.lit(proposal_job_run_id))
    ...
    for nb_path, nb_label in PIPELINE_NOTEBOOKS:
        ...
        try:
            dbutils.notebook.run(nb_path, timeout_seconds=1800,
                                 arguments=proposal_pipeline_args)
    ...
    try:
        ...
        snapshot_silver_tables(case_folder, batch_id, proposal_job_run_id)
