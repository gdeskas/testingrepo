...
def retrieve_similar_chunks(
    ...
    """
    Retrieve similar chunks using Mosaic AI Vector Search.
    """
    # Get the index
    ...
    # Search the index (get extra results for filtering)
    search_k = k * 5 if exclude_duplicates else k * 2
    
    results_raw = index.similarity_search(
        query_vector=q_vec,
    ...
    for row in results_raw.get("result", {}).get("data_array", []):
        chunk_id, chunk_cid, section, chunk_idx, text, score = row[0], row[1], row[2], row[3], row[4], row[5]
