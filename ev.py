...
# MAGIC %md
# MAGIC # LLM Extraction — Consistency Evaluation
# MAGIC
# MAGIC **How to use:**
# MAGIC 1. In **Section 1**, uncomment the one `VOLUME_ROOT` you want to evaluate.
# MAGIC 2. Hit **`Run All`** — the notebook will stage, ingest, run the pipeline, then evaluate with 5× LLM runs.
# MAGIC 3. Repeat for the next example by uncommenting a different line and running again.
# MAGIC
# MAGIC Each run appends to the review Delta table and creates a new MLflow experiment run,
# MAGIC so all your results accumulate across examples.
...
# MAGIC %md
# MAGIC ---
# MAGIC ## 1. Select Proposal Folder
# MAGIC Uncomment **one** `VOLUME_ROOT` line at a time. All 20 examples are listed below.

# COMMAND ----------

# DBTITLE 1,Uncomment one VOLUME_ROOT to evaluate
ADLS_BASE = "abfss://comm-afl-lz@cbssstuksdbr01dev.dfs.core.windows.net/raw-data/Example Docs - Latest"

VOLUME_ROOT = f"{ADLS_BASE}/01. JDN Logistics Ltd - HSH Business Finance - AFS"
# VOLUME_ROOT = f"{ADLS_BASE}/02. KD Grab HIre Limited - Anglo Scottish"
# VOLUME_ROOT = f"{ADLS_BASE}/03. West Country Recovery (SW) Ltd - EFT Finance"
# VOLUME_ROOT = f"{ADLS_BASE}/04. CDT Electrical Ltd - Motion"
# VOLUME_ROOT = f"{ADLS_BASE}/05. TLB Deliveries Ltd - Holmesdale"
# VOLUME_ROOT = f"{ADLS_BASE}/06. Electric Access Solutions Ltd - Evolution"
# VOLUME_ROOT = f"{ADLS_BASE}/07. CJT Vocational Skills Ltd - Crown Business Finance - AFS"
# VOLUME_ROOT = f"{ADLS_BASE}/08. Duffield Harrison LLP - Audeo FS Ltd"
# VOLUME_ROOT = f"{ADLS_BASE}/09. Glass Padel Club Ltd - White Rose Finance Group Ltd"
# VOLUME_ROOT = f"{ADLS_BASE}/10. Your Shortlist Original Ltd - Clear Asset Finance"
# VOLUME_ROOT = f"{ADLS_BASE}/11. Beachers Coaches Ltd - Fundi - AFS"
# VOLUME_ROOT = f"{ADLS_BASE}/12. Central England Healthcare - LDF"
# VOLUME_ROOT = f"{ADLS_BASE}/13. Rural GM Ltd - FMF Group Ltd - AFS"
# VOLUME_ROOT = f"{ADLS_BASE}/14. Stellar"                                         # TODO: complete name
# VOLUME_ROOT = f"{ADLS_BASE}/15. ..."                                             # TODO: add folder name
# VOLUME_ROOT = f"{ADLS_BASE}/16. ..."
# VOLUME_ROOT = f"{ADLS_BASE}/17. ..."
# VOLUME_ROOT = f"{ADLS_BASE}/18. ..."
# VOLUME_ROOT = f"{ADLS_BASE}/19. ..."
# VOLUME_ROOT = f"{ADLS_BASE}/20. ..."

case_folder = VOLUME_ROOT.rstrip("/").split("/")[-1]
print(f"Selected: {case_folder}")
...
# MAGIC %md
# MAGIC ---
# MAGIC ## 2. Stage & Ingest Files from ADLS into Bronze
...
# DBTITLE 1,Stage files from ADLS to DBFS volume
SRC_DIR = VOLUME_ROOT
...
file_count = 0
for f in dbutils.fs.ls(SRC_DIR):
    if f.isDir():
        continue
    new_name = safe_name(f.name)
    dst_path = f"{DST_DIR}/{new_name}"
    print(f"COPY: {f.name}  ->  {case_folder}/{new_name}")
    dbutils.fs.cp(f.path, dst_path, True)
    file_count += 1

print(f"\nDone. Files staged to: {DST_DIR}")

# COMMAND ----------

# DBTITLE 1,Set staged proposal path
staged_proposal = f"{STAGING_ROOT}/{case_folder}"

# COMMAND ----------

# DBTITLE 1,Ingest into Bronze Delta table
binary_df = (
    spark.read.format("binaryFile")
    .option("recursiveFileLookup", "true")
    .load(staged_proposal)
    .withColumn("folder_path", F.lit(staged_proposal))
    ...
    .withColumn(
        "proposal_id",
        F.sha2(F.concat_ws("||", F.lit(staged_proposal), F.current_timestamp().cast("string")), 256)
    )
...
print(f"Ingested {row_count} files from {case_folder}")

# Overwrite working table (pipeline expects single proposal)
files_loaded_df.write.mode("overwrite").option(
    "overwriteSchema", "true"
).saveAsTable(f"`{catalog}`.`{brz_schema}`.{working_target_table}")

# Append to storage table
files_loaded_df.write.mode("append").option(
    "mergeSchema", "true"
).saveAsTable(f"`{catalog}`.`{brz_schema}`.{storage_target_table}")

print(f"Written to `{catalog}`.`{brz_schema}`.{working_target_table}`")

# COMMAND ----------

# DBTITLE 1,Preview loaded files
display(files_loaded_df.select("PROPOSAL_ID", "DOCUMENT_ID", "FILE_NAME", "FILE_EXT", "ADLS_SOURCE_PATH", "LENGTH"))
...
# MAGIC Runs the 5 pipeline notebooks in sequence to extract text and categorise documents.

# COMMAND ----------

# DBTITLE 1,Run pipeline notebooks
pipeline_args = {
    "catalog": catalog,
...
print("4/5 03.categorise_image — OK")
...
print("5/5 04.categorise_other_doc_type — OK")

print(f"\nPipeline complete for: {case_folder}")

# COMMAND ----------

# DBTITLE 1,Verify gddoc_category is populated
...
print(f"Found {proposal_count} proposal(s) in `{catalog}`.`{slr_schema}`.{source_table}`")
